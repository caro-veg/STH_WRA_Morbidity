...
file <- "fertilisedFemaleWorms_old_50-70_treat2x.RData"
stub <- substring(file, first=22, last=45)
results <- get(load(paste0(path, file)))


###############################################################################################
# Figure 1a: 20-50% prevalence treat 2x per year, ICL model
###############################################################################################
# read in data
###############################################################################################
...
file3 <- "eggCounts_50-100_untreated.RData"
...
df.mhi.icl.50_100 <- rbind(df.mhi.icl.untreat, df.mhi.icl.old, df.mhi.icl.new)
df.mhi.icl.50_100$scenario <- factor(df.mhi.icl.50_100$scenario, levels=c("new", "old", "untreated"))

p1 <- ggplot(df.mhi.icl.50_100)
...
p1 <- p1 + scale_y_continuous(name="% women with MHI", expand=c(0, 0)) + coord_cartesian(ylim=c(0,23))
print(p1)
...
df.mhi.emc.50_100 <- rbind(df.mhi.emc.untreat, df.mhi.emc.old, df.mhi.emc.new)
df.mhi.emc.50_100$scenario <- factor(df.mhi.emc.20_50$scenario, levels=c("new", "old", "untreated"))

p2 <- ggplot(df.mhi.emc.50_100)
...
p2 <- p2 + scale_y_continuous(name="% women with MHI", expand=c(0, 0)) + coord_cartesian(ylim=c(0,23))
print(p2)

pic_mhi_means <- plot_grid(p1, p2, labels=c('a', 'b'), label_size=18, ncol=2, hjust=0, vjust=1.0)
